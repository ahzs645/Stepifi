...
function parseStl(buffer: ArrayBuffer): THREE.BufferGeometry {
  ...
  const isBinary = !header.startsWith('solid') ||
    // Some binary files start with "solid" by coincidence
    (buffer.byteLength > 84 && buffer.byteLength === 84 + dataView.getUint32(80, true) * 50);
...
function parseAsciiStl(buffer: ArrayBuffer): THREE.BufferGeometry {
  ...
  while ((normalMatch = normalPattern.exec(text)) !== null) {
    ...
    // Find the next 3 vertices after this normal
    for (let i = 0; i < 3; i++) {
