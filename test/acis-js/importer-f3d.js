...
export async function importF3D(fileData, oc, options = {}) {
  ...
  let ACISBinaryReader
  try {
    const acisModule = await import('./reader.js')
    ACISBinaryReader = acisModule.ACISBinaryReader
  } catch (e) {
    console.warn('ACIS reader not available:', e)
  }

  // Create ACIS reader factory
  const acisReaderFactory = ACISBinaryReader
    ? data => new ACISBinaryReader(data)
    : null
  ...
  if (buildBody && oc) {
    for (const smb of f3dData.smbFiles) {
      if (smb.isRaw) {
        ...
        if (ACISBinaryReader) {
          try {
            const reader = new ACISBinaryReader(smb.data)
            if (reader.readBinary()) {
